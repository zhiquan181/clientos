<?php
	...
	if ($company&&$pic_str!=null) {
		...
		if ($row['phone']!=$phone) {
		    ...
		    $sql2 = "INSERT INTO client VALUES (null,'{$grade}','{$company}','{$client}','{$phone}','{$population}','{$demand}','{$wxvisit}','{$course}','{$remarks}','{$others}','{$lastdate}','{$remind}','{$pic_path}','{$abstract}',1)";
		    $rw = $pdo->exec($sql2);

		    if ($rw > 0) {
		        echo "<script> alert('保存成功，返回系统首页');
		                        window.location='system.php'; //跳转到注册页
		             </script>";
		    } else {
		        echo "<script> alert('保存失败，返回新增客户页面');
		...
		if ($row['phone']!=$phone) {
		    ...
		    $sql2 = "INSERT INTO client VALUES (null,'{$grade}','{$company}','{$client}','{$phone}','{$population}','{$demand}','{$wxvisit}','{$course}','{$remarks}','{$others}','{$lastdate}','{$remind}',null,'{$abstract}',1)";
		    $rw = $pdo->exec($sql2);
